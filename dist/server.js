...
import { z } from "zod";
import { getFormById, listForms, queryFieldValues, listClients, toFormSummary, } from "./db.js";
export function createMcpServer() {
    const server = new McpServer({ name: "cosito-mcp", version: "1.0.0" });
    server.tool("get_form", "Retrieve a single quality inspection form by its unique ID. Returns the complete form including all sections, fields, and recorded answers.", { form_id: z.string().describe("Unique ID of the inspection form") }, async ({ form_id }) => {
    ...
    server.tool("list_forms", "List inspection forms with optional filters. Returns form summaries by default; pass include_sections=true for full data. Use this to browse forms, find forms for a client or product, or look up forms within a date range.", {
    ...
    server.tool("list_clients", "Return a sorted list of all unique client names present in the database. Useful for discovery before filtering by client.", {}, async () => {
