...
async function storeTokens(sub: string, refreshToken: string, accessToken: string, expiresIn: number): Promise<void> {
...
async function getStoredTokens(sub: string): Promise<Record<string, unknown> | null> {
...
async function refreshAccessToken(sub: string): Promise<string | null> {
  ...
  const newRefreshToken = (tokens.refresh_token ?? stored.refresh_token) as string;
  await storeTokens(sub, newRefreshToken, tokens.access_token as string, tokens.expires_in as number);
...
export const handler = async (event: FnUrlEvent): Promise<LambdaResult> => {
  ...
  if (path === "/token" && method === "POST") {
    ...
    if (cognitoRes.ok) {
      try {
        const tokenJson = JSON.parse(responseBody) as Record<string, unknown>;
        if (tokenJson.refresh_token && tokenJson.access_token) {
          const idPayload = decodeJwtPayload(tokenJson.id_token as string ?? "");
