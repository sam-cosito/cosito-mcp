...
export async function getFormById(formId: string): Promise<InspectionForm | null> {
  const docClient = await getDocClient();
  const response = await docClient.send(
    new GetCommand({ TableName: TABLE_NAME, Key: { form_id: formId } })
  );
  return (response.Item as InspectionForm) ?? null;
}

export async function listForms(filters: FormQueryFilters): Promise<InspectionForm[]> {
  const docClient = await getDocClient();
  ...
  if (client) {
    ...
    if (date_from && date_to) {
      ...
      (params.ExpressionAttributeValues as Record<string, unknown>)[":from"] = date_from;
      (params.ExpressionAttributeValues as Record<string, unknown>)[":to"] = date_to;
    } else if (date_from) {
  ...
  let forms = raw as unknown as InspectionForm[];

  if (item) {
    const needle = item.toLowerCase();
    forms = forms.filter((f) =>
      f.items?.some((i) => i.toLowerCase().includes(needle)) ||
  ...
  if (inspection_status) {
    ...
    forms = forms.filter(
      (f) => f.quality_assessment?.inspection_status?.toLowerCase() === needle
    );
  }
...
export async function getQualityMetrics(filters: FormQueryFilters): Promise<QualityMetricResult[]> {
...
export async function listClients(): Promise<string[]> {
  ...
  const unique = [...new Set(raw.map((r) => r["client"] as string).filter(Boolean))];
  return unique.sort();
}

export function toFormSummary(form: InspectionForm): FormSummary {
  return {
